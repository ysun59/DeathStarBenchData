...
#第一秒all的cpu utilization, core 0的cpu utilization， core 1的cpu utilization....core31的cpu utilization，
#第二秒all的cpu utilization, core 0的cpu utilization， core 1的cpu utilization....core31的cpu utilization，
#...
#第60秒all的cpu utilization, core 0的cpu utilization， core 1的cpu utilization....core31的cpu utilization，
def mpstatData():
    ...
    if os.path.exists(cpu_perf_path):
      ...
      with open(cpu_perf_path, 'r') as file:
        ...
        for line in lines:
          ...
          # print(a)
          # print(a[1:2])
          if a[1:2] == ['CPU'] or a[1:2] == [] or a[1:2] == ['5.4.0-124-generic'] or a[1:2] == ['5.4.0-126-generic'] or a[0:1] == ['Linux']:
            continue
          other=str(100 - Decimal(a[11]))
          lineBuf.append(other)
          # print(lineBuf)
...
#print the avg of all's usr%, sys%, soft%, guest%, other% during 60s(pic below)
#60秒all的 usr% 的平均值
#60秒all的 sys% 的平均值
#60秒all的 soft% 的平均值
#60秒all的 guest% 的平均值
    ...
    if os.path.exists(mpstat_csv_path):
      ...
      with open(mpstat_csv_path, 'r') as file:
        ...
        for i in range(0,rowNum):
          ...
          avg = summ/colNum
          outputs.append(avg)
        print("avg usr%, sys%, soft%, guest%, other% during 60s are:")
        print(outputs)
        print("")

      mpstat_csv_path = os.path.join('.', folder, 'mpstat_all_Avg.txt')
      with open(mpstat_csv_path, 'wt') as file:
        # file.write('\n'.join(str(i) for i in outputs)) #用这个就是竖着输出了,可能竖着输出更好
        file.write(','.join(str(i) for i in outputs))
